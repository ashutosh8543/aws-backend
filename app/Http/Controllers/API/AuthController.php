...
class AuthController extends AppBaseController
{
    ...
    {  
        ...
        $user = User::where('email',$email)->where('role_id',$role_id)->first();
        if (empty($user)) {
        ...
        if(!$user->status){
            return $this->sendError('You account is not active.', 422);
        }
        
        if(Auth::attempt(['role_id'=>$role_id,'email' => $email, 'password' => $password])){
