...
class RoleAPIController extends AppBaseController
{
    ...
    public function index(Request $request): RoleCollection
    {
        ...
        $roles = $this->roleRepository->where('id','>',$userdetails->role_id)->where('status',1)->paginate($perPage);
    ...
    public function LoginRole(Request $request): RoleCollection
    {
        $perPage = getPageSize($request);
        $userdetails =Auth::user();
        $roles = $this->roleRepository->where('status',1)->get();
        RoleResource::usingWithCollection();
        return new RoleCollection($roles);
    }
