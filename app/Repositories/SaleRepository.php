...
class SaleRepository extends BaseRepository
{
    ...
    public function storeSale($input): Sale
    {
        try{
            ...
            $QuotationId = $input['quotation_id'] ?? false;
            $saleInputArray = Arr::only($input, [
                'customer_id','country','salesman_id', 'warehouse_id', 'tax_rate','image','location', 'tax_amount', 'discount', 'shipping', 'grand_total',
                'received_amount', 'paid_amount','sale_type', 'payment_type', 'note', 'date', 'status', 'payment_status',
            ]);
        $saleInputArray['user_id'] = Auth::id();
        ...
        $customerDetails=Customer::find($input['customer_id']);
        if($input['payment_type']==5){
                $total_payment=$input['paid_amount'];
                if($customerDetails->credit_limit > $total_payment){
                   $updatedCredits=$customerDetails->credit_limit-$total_payment;
                   $customerDetails->update(['credit_limit'=>$updatedCredits]);
                  $credits= CreditLimit::create([
                ...
                $collection->update(['unique_code'=>'CLN_'.$collection->id]);
                $credits->update(['unique_code'=>'CROUT_'.$credits->id]);
                }else{
                 throw new UnprocessableEntityHttpException('Customer Credit limit is less than total Payment');
             }
        ...
        foreach ($input['sale_items'] as $saleItem){
            ...
            $loadproduts = LoadProductToSaleman::where('salesman_id',$input['salesman_id'])
            ->whereProductId($saleItem['product_id'])
            ->whereDate('assign_for_date',$input['date'])
            ->first();
            if($loadproduts  && $loadproduts->quantity >= $saleItem['quantity']){
                $totalQua = $loadproduts->quantity - $saleItem['quantity'];
                $loadproduts->update([
                    'quantity' => $totalQua,
                ]);
            }else{
    ...
    public function storeSaleItems($sale, $input)
    {
        ...
        if ($input['payment_status'] == Sale::PAID) {
            $input['paid_amount'] = $input['paid_amount'];
            SalesPayment::create([
                'sale_id' => $sale->id,
