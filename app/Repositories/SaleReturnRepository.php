    ...
    public function storeSaleReturn($input): SaleReturn
    {
        try {
            ...
            $saleReturnInputArray = Arr::only($input, [
                'customer_id','country', 'warehouse_id', 'tax_rate', 'tax_amount', 'discount', 'shipping','image', 'grand_total',
                'paid_amount', 'payment_type','return_type', 'note', 'date', 'status', 'sale_id',
            ]);                        
            ...
            $credits->update(['unique_code'=>'CRIN_'.$credits->id]);
    ...
    public function storeSaleReturnItems($saleReturn, $input)
    {
        foreach ($input['sale_return_items'] as $saleReturnItem) {
            ...
            $saleOfProduct = SaleItem::where('product_id', $saleReturnItem['product_id'])
               ...
               $updateremaining_quantity = SaleItem::where('product_id', $saleReturnItem['product_id'])
                ->whereHas('sale', function (Builder $query) use ($input) {
                    $query->where('id', $input['sale_id']);
                })
                ->first();
                $updateremaining_quantity->update(["quantity"=>$updateremaining_quantity->quantity-$saleReturnItem['quantity']]);
